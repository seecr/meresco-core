...
class LoggingComposer(object):
    def __init__(self, unqualifiedTermFields):
        self._unqualifiedTermFields = unqualifiedTermFields
        self._queries = {}

    def addQuery(self, field, value):
        if field not in self._queries:
            self._queries[field] = []
        self._queries[field].append(value)
    ...
    def compose(self, node):
        ...
        if node.__class__ == SEARCH_CLAUSE:
            if len(node.children()) == 1: #unqualified term
                ...
                self.addQuery("unqualified", unqualifiedRhs)
